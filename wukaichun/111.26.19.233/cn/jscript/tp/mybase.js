...
$(function() {
	$(':text, :password').addClass('ui-widget ui-widget-content ui-corner-all');
	$('.button').css({
		'padding' : '0'
	}).button();
	$(':submit').button();
	if ($.browser.msie) {
		// fix the position BUG under IE6
		$('.button, :submit').addClass("ui-state-hover");
		setTimeout(function() {
			$('.button, :submit').removeClass("ui-state-hover");
		}, 100);
	}

	var query_fields = $('.query-fields');
	if (query_fields && query_fields.length) {
		query_fields.remove();
		var queryBtn = $(
				'<a class="action ui-state-default ui-corner-all" href="#" onclick="return false;">'
		...
		$('#sitemap-nav').append(queryBtn).append(
				'<div style="clear:both;"></div>');
		$('.ui-icon', queryBtn).css({
			'margin' : '0 0 0 -18px',
			'position' : 'absolute'
		});
	}
	//
	$('.data-table tr').mouseover(function() {
		$(this).addClass('hl-row');
	}).mouseout(function() {
		$(this).removeClass('hl-row');
	});

	$('.data-table tr').attr("align", "center");
	$('.data-table tr:even').addClass("even-row");
	$('.data-table tr:odd').addClass("odd-row");

	$('.data-table1 tr').mouseover(function() {
		$(this).addClass('hl-row');
	}).mouseout(function() {
		$(this).removeClass('hl-row');
	});

	$('.data-table1 tr').attr('height', '25px');
	$('.data-table1 tr:even').addClass("even-row");
	$('.data-table1 tr:odd').addClass("odd-row");
});